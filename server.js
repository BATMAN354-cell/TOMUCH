...
app.get('/pair', async (req, res) => {
    ...
    try {
        ...
        // Slight delay to ensure socket registration is stable
        if(!globalSock.authState.creds.registered) {
            setTimeout(async () => {
                try {
                    ...
                    console.log("Delayed pairing error:", err);
                }
            }, 3000);
        }
...
async function startBatmanBot() {
    ...
    const sock = makeWASocket({
        ...
        auth: state,
        browser: Browsers.ubuntu("Chrome") // Using official Baileys Browser helper to bypass restriction
    });
...
_"Never go back on your word."_`;
            ...
            if (command === '.mute' && isGroup) {
                ...
                return reply('🔒 Group muted by FAMOS BATMAN BOT.');
            }
